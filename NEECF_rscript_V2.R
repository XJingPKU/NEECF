...
# write.csv(df.enz.neecf.summary, "./outputs/NEECF_data_summary.csv")
...
enz.df.act.aovftest$ypos <- rep(c(-1.0, 4.65, 3.0, -1.0, 6.0, -2.0, 2.2), 4)
...
enz.df.act.aovftest <- ddply(enz.df.act, c("fD", "variable"), aovFtest.mat)
enz.df.act.aovftest$labels <- with(enz.df.act.aovftest,
                                   paste("F = ", F, "p =", P))
...
# by depth
# enzyme activity
ggplot(enz.df.act, aes(x = MAT, y = value, color = fD)) +
  geom_point(size = 2.5, shape = 1) +
  geom_smooth(method = 'lm', color = "black", size = 0.4, se = FALSE) +
  facet_grid(variable ~ fD, scales = "free_y") +
...
enz.df.act.aovftest <- ddply(enz.df.act, c("fD", "variable"), aovFtest.map)
enz.df.act.aovftest$labels <- with(enz.df.act.aovftest,
                                   paste("F = ", F, "p =", P))
...
# by depth
# enzyme activity
ggplot(enz.df.act, aes(x = MAP, y = value, color = fD)) +
  geom_point(size = 2.5, shape = 1) +
  geom_smooth(method = 'lm', color = "black", size = 0.4, se = FALSE) +
  facet_grid(variable ~ fD, scales = "free_y") +
...
# ratio
# aov.ftest to corect F-test
enz.df.ratio.aovftest <- ddply(enz.df.ratio, c("fD", "variable"), aovFtest.mat)
...
ggplot(enz.df.ratio, aes(x = MAT, y = value, color = fD)) +
  geom_point(size = 2.5, shape = 1) +
  geom_smooth(method = 'lm', color = "black", size = 0.4, se = FALSE) +
  facet_grid(variable ~ fD, scales = "free_y") +
  labs(x = expression("Mean annual temperature ("*degree*C*")"), 
...
# ratio
# aov.ftest to corect F-test
enz.df.ratio.aovftest <- ddply(enz.df.ratio, c("fD", "variable"), aovFtest.map)
enz.df.ratio.aovftest$labels <- with(enz.df.ratio.aovftest,
                                     paste("F = ", F, "p =", P))
enz.df.ratio.aovftest$xpos <- rep(1400, 12)
enz.df.ratio.aovftest$ypos <- rep(c(-0.2, 0.15, -0.25), 4)
ggplot(enz.df.ratio, aes(x = MAP, y = value, color = fD)) +
  geom_point(size = 2.5, shape = 1) +
  geom_smooth(method = 'lm', color = "black", size = 0.4, se = FALSE) +
  facet_grid(variable ~ fD, scales = "free_y") +
  labs(x = "Mean annual precipitation (mm)", 
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# CN vs CP ratio
ddply(enz.neecf, "fD", summarise, 
      mean.CN = mean(CNratio, na.rm = T),
      sd.CN = sd(CNratio, na.rm = T),
      mean.CP = mean(CPratio, na.rm = T),
      sd.CP = sd(CPratio, na.rm = T))
...
boxplot(value ~ site*fN*fD, 
        data = enz.df %>% filter(value != 0 & variable == "PHOS"))
enz.df %>%
  filter(value != 0 & variable == "PHOS") %>%
  ggplot(aes(x = fN, y = value)) +
  geom_boxplot() +
  facet_grid(site ~ fD, scales = "free_y") +
  labs(x = "N treatment",
       y = "Phosphotase activity") +
  theme_bw(base_size = 9) +
  theme(strip.background = element_blank(),
        legend.position = "none",
        axis.title = element_text(size = 12))
ggsave("./outputs/NEECF_PHOS_site_depth.pdf", height = 8, width = 6)
